...
int RusSwitch1Pin = 46;
int RusState1Pin = 40;
int RusSwitch2Pin = 47;
int RusState2Pin = 41;
bool bRusL1On = false;
/**/
bool bRusL2On = false;
...
unsigned long RusLivoloTimeConst = 15000; //time, when system is ignoring PIR after manual
                        //turn on/off the light --MAYBE,NOT SURE
unsigned long RusPIRTimeConst = 3000;//time, when system is ignoring PIR after PIR-controlled
                     //turn on/off the light --MAYBE,NOT SURE
...
void loop() {
  ...
  if (((millis() - lastRusLivoloEvent) > RusLivoloTimeConst) || start_flag)
  {
    state = digitalRead(RusPIRPin);
    if (debug2)
    {
      Serial.println("in upper pir stage");
    }
